...
#      You can uncomment them to see what they do. The explanation is above or behind the block of code.
#    - This file is only an example of the basic renderer. You can find ray-tracing examples in the other scenes.
...
# change_cursor('Window1', 'crosshair')  # Change cursor to one of Tkinter's builtin cursors

camera = Camera(0, 0, -3)
camera.displaying = "Window1"  # (window-name)
camera.on()  # To display, it needs to be on
camera.visible = True  # Makes it visible to other cameras
camera.box_size = 1  # Size of 'box' that represents camera when viewing from other camera
...
#                         shading=True)
#             # This is are new instance-variables, they are not built-in
...
Plate([(3, 1, 1)], fill='red', smart_coords=False, radius=0.05, outline='red')
...
controller.bind('mouse_y', 'x_rotation', -0.5)  # 0.5 is movement-speed
controller.bind('mouse_x', 'y_rotation', 0.5)
controller.bind('z', 'forward', 0.5)
controller.bind('q', 'left', 0.5)
controller.bind('s', 'backwards', 0.5)
controller.bind('d', 'right', 0.5)
controller.bind('Key.space', 'y', 0.5)
controller.bind('Key.shift', 'y', -0.5)
# controller.notify('a', 'toggle', 'test')  # key, toggle or hold, variable name (to see value of variable, eg. use
                                                                                # controller.test)
